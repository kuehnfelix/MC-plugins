...
public class Flummi extends JavaPlugin implements Listener {
    ...
    public void onEntityShootBowEvent(EntityShootBowEvent event) {
        ...
        if (((entity instanceof Player)) && (projectile.getType() == EntityType.ARROW)) {
            ...
            if (event.getBow().getEnchantmentLevel(Enchantment.SILK_TOUCH) > 0) {
                if (player.hasPermission("flummi.arrow.use")) {
                    projectile.setMetadata("bouncing", new FixedMetadataValue(this, true));
                }

                if (player.hasPermission("flummi.aim")) {
    ...
    public void onProjectileLaunch(ProjectileLaunchEvent event) {
        ...
        if (type == EntityType.SNOWBALL && projectile.getShooter() instanceof Player) {
            ...
            if (player.hasPermission("flummi.snowball.use")) {
                projectile.setMetadata("bouncing", new FixedMetadataValue(this, true));
            }
        }
    }
    ...
    public void onProjectileHitEvent(ProjectileHitEvent event) {
        ...
        if (true || projectile.hasMetadata("bouncing")) {
            ...
            if (speed < 0.3D || (projectileType == EntityType.ARROW && speed < 0.5D)) {
                if(projectile instanceof Snowball) {
                    ItemStack it = new ItemStack(Material.SNOW_BALL);
                    ItemMeta meta = it.getItemMeta();
                    meta.setDisplayName("FLUMMI");
                    meta.addEnchant(Enchantment.ARROW_KNOCKBACK, 2, true);
                    it.setItemMeta(meta);
                    //Bukkit.getWorld("world").dropItem(projectile.getLocation(), new ItemStack(Material.SNOW_BALL));
                }
                return;
            }
